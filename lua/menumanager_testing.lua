...
--Returns: nil
function KineticHUD:OLDCreatePanels()
...
function KineticHUD:CreateWorldWorkspace(panel_data)
	local panel_w = panel_data.panel_w
	local panel_h = panel_data.panel_h
	local panel_name = tostring(panel_data.name)
	local debug_text = panel_data.debug_text
	local debug_color = panel_data.debug_color
	local debug_alpha = panel_data.debug_alpha
	local panel_layer = panel_data.panel_layer
	
	local halign = panel_data.halign
	local valign = panel_data.valign
...
--			ws = managers.gui_data:create_fullscreen_workspace("ws_" .. panel_name,self._gui)
		...
		if ws then 
			...
			if halign == "right" then 
				panel:set_x(ws:panel():w() - panel_w)
			elseif halign == "center" then 
				panel:set_x((ws:panel():w() - panel_w) / 2)
			end
			if halign == "bottom" then 
				panel:set_y(ws:panel():h() - panel_h)
			elseif halign == "center" then 
				panel:set_y((ws:panel():h() - panel_h) / 2)
			end
			--]]
		else
			self:c_log("ERROR: No workspace for world panel with id " .. tostring(i))
	...
	if ws and panel then 
		...
		local debug_rect = self.make_debug_rect(panel,debug_alpha,debug_color)
		debug_rect:set_layer(-1)
...
function KineticHUD:LinkWS(link_target_object)
	...
	for workspace_id,workspace_data in pairs(KineticHUD._workspaces) do 
		local ws = workspace_data.workspace
		local params = workspace_data.params
		
		local panel_w = params.panel_w
		local panel_h = params.panel_h
		
		local halign = params.halign
		local valign = params.valign
			
		local yaw = params.rotation_yaw
		local pitch = params.rotation_pitch
		local roll = params.rotation_roll
		--pitch and roll are not yet implemented but i suspect i could skip a lot of pain with mvector3.rotate_with()
		
		local offset_x = params.position_x
		local offset_y = params.position_y
		local offset_z = params.position_z
		...
		local _fwd = camera:position() + (camera:rotation():y() * distance)
			
		local world_w = mvector3.length(_right - origin)
		local world_h = mvector3.length(_bottom - origin)
		...
		if halign == "right" then 
			...
			local _x = math.cos(yaw) * world_w
			local _y = math.sin(yaw) * world_w
...
--Returns: bool
function KineticHUD:OLDLinkWS(link_target_object)
...
--	local selected_parent_panel = self._world_panels[layout_settings.player_weapons_panel_location]
	...
	local selected_parent_panel = self:CreateWorldWorkspace({
		self.workspace_values.DEFAULT
	})
